...
void MenuWidget::slotCheckActiveItem()
{
    ...
    if (item) {
        buttonBelow = qobject_cast<QToolButton*>(item->toGraphicsObject());
    }
...
{
    m_currentButton = qobject_cast<QToolButton*>(sender());

    if (m_currentButton) {
        m_currentButton->setDown(true);
    }
    m_visibleMenu = showMenu();
